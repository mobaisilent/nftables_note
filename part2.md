...
nftables并没有严格定义五条链，但是在常见的网络防火墙设置中，我们通常会看到以下五条链：

1. `PREROUTING`：这条链用于处理到达网络接口，但还未路由到目标的数据包。在这个阶段，通常会进行目标地址转换（Destination NAT）。
2. `INPUT`：这条链用于处理路由到本机的数据包。在这个阶段，通常会进行访问控制，例如拒绝或接受特定的流量。
3. `FORWARD`：这条链用于处理路由通过本机的数据包。在这个阶段，通常会进行访问控制，例如拒绝或接受特定的流量。
4. `OUTPUT`：这条链用于处理本机产生的数据包。在这个阶段，通常会进行访问控制，例如拒绝或接受特定的流量。
5. `POSTROUTING`：这条链用于处理离开网络接口的数据包。在这个阶段，通常会进行源地址转换（Source NAT）。

这五条链分别对应于数据包处理的五个阶段，可以用来实现各种复杂的网络防火墙规则。
